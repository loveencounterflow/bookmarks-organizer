...
create function _X_.demo() returns void language plpgsql as $$
  begin
    perform log();
...
do $$ begin perform _X_.demo(); end; $$;

-- select * from unit_units where name ~ 'quart';

-- ---------------------------------------------------------------------------------------------------------
create function _X_.set_prefix( ¶prefix text, ¶factor double precision ) returns void language plpgsql as $$
  begin
...
-- ---------------------------------------------------------------------------------------------------------
create function _X_.set_unit( ¶name text, ¶unit unit ) returns void language plpgsql as $$
  begin
...
-- insert into unit_prefixes values ('foo', 42) on conflict do nothing;
do $$ begin perform _X_.set_prefix( 'foo',        108       ); end; $$;
do $$ begin perform _X_.set_unit(   'legobrick',  '9.6 mm'  ); end; $$;
do $$ begin perform _X_.set_unit(   'Spanne',     '1|12 m'  ); end; $$;
select '1 foobar'::unit;
select '1 m'::unit @ 'legobricks' as one_meter;
select '1|3 m'::unit @ 'Spanne' as one_meter;
select sqrt( '47m²'::unit );
